...
(async () => {
  db = await open({
    filename: './BD4AS1/database.sqlite',
...
app.get('/restaurants/details/:id', async (req, res) => {
  ...
  try {
    ...
    if (results.restaurants.length === 0) {
      return res.status(404).json({
        message: 'No Restaurants Found with ' + id,
      });
    }
    return res.status(200).json(results);
  } catch (error) {
...
app.get('/restaurants/filter', async (req, res) => {
  ...
  try {
    const results = await fetchRestaurantsByFilter(
      isVeg,
      hasOutdoorSeating,
      isLuxury
    );
    if (results.restaurants.length === 0) {
      return res.status(404).json({
        message: 'No Restaurants Found with these filters.',
      });
    }
    return res.status(200).json(results);
  } catch (error) {
...
app.get('/restaurants/sort-by-rating', async (req, res) => {
  try {
    const results = await sortRestaurantsByrating();
    if (results.restaurants.length === 0) {
      return res.status(404).json({
        message: 'No Restaurants Found ',
      });
...
app.get('/dishes/details/:id', async (req, res) => {
  let id = req.params.id;
  try {
    const results = await fetchDishesById(id);
    if (results.dishes.length === 0) {
      return res.status(404).json({
...
app.get('/dishes/filter', async (req, res) => {
  ...
  try {
    ...
    if (results.dishes.length === 0) {
      return res.status(404).json({
        message: 'No dishes Found with These filter.',
      });
    }
    return res.status(200).json(results);
  } catch (error) {
